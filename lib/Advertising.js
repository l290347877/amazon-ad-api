...
class Advertising {
  ...
  constructor(config){
    ...
    this._big_int_keys = ['campaignId', 'keywordId', 'adId', 'adGroupId', 'targetId', 'profileId', 'portfolioId'];
    this._param_regex = {
      '{profileId}':'[0-9]*',
      '{campaignId}':'[0-9]*',
      '{adGroupId}':'[0-9]*',
      ...
      '{adId}':'[0-9]*',
      '{targetId}':'[0-9]*',
      '{recordType}':'(campaigns|adGroups|keywords|productAds|ads|asins|targets)',
      '{reportId}':'[^/]*',
      '{snapshotId}':'[0-9]*',
      '{budgetRuleId}':'[^/]*',
  ...
  _createRequest(req_params){
    return {
      ...
      headers:['/v2/profiles', '/testAccounts'].includes(req_params.operation) ? this._addHeaders() : this._addHeaders(true),
  ...
  _isUnauthorized(json_res, is_v3 = false){
    ...
    return ((!is_v3 && json_res.message === 'Unauthorized') || (json_res.code === 'UNAUTHORIZED' && json_res.details === 'HTTP 401 Unauthorized'));
  }

  _parseResult(res){
    try {
      let body = res.body !== '' ? JSONbig.parse(res.body) : res.body;
      if (res.statusCode === 401 && ((body === '' && res.headers['www-authenticate'].includes('authorized customer is null')) || body.details === 'Authentication failed')) {
        return {
          code:'UNAUTHORIZED',
      ...
      if ([429, 500].includes(res.statusCode) && this._options.auto_request_throttled){
        ...
        if (this._options.max_retries && tries > this._options.max_retries){
          throw new CustomError({
            ...
            message:'Could not receive valid profiles in time, too many retries (> ' + this._options.max_retries + ' tries)!'
          });
        }
  ...
  async _wait(tries){
    // 生成 110 到 140 之间的随机整数
    const minWaitTime = 40;
  ...
  async _getReportURL(report_id, report_path, is_v3, tries = 0){
    ...
    if (['SUCCESS','SUCCESSFUL','COMPLETED'].includes(res.status)){
      if ((!res.location || res.location === '') && (!res.url || res.url === '')){
        throw new CustomError({
  ...
  _addDefaultMetrics = (req_params) => {
    ...
    if (req_params.metricsType){
      ...
      if (req_params.segmentMetricsType){
        return record_type_infos.segment[req_params.body.segment].metrics[req_params.segmentMetricsType];
      }
  ...
  async _getReportV3(req_params){
    ...
    if (req_params.recordType === 'campaigns' || req_params.recordType === 'adGroups'){
    ...
    } else if (req_params.recordType === 'keywords' || req_params.recordType === 'targets'){
    ...
    if (req_params.recordType === 'keywords' || (req_params.recordType === 'asins' && req_params.metricsType === 'keywords')){
      configuration.filters = [{
        field:'keywordType',
        values:['BROAD','PHRASE','EXACT']
      }];
    } else if (req_params.recordType === 'targets' || (req_params.recordType === 'asins' && req_params.metricsType === 'targets')){
      configuration.filters = [{
        field:'keywordType',
    ...
    if (req_params.body?.reportDate){
      let new_date_format = req_params.body.reportDate.slice(0, 4) + '-' + req_params.body.reportDate.slice(4, 6) + '-' + req_params.body.reportDate.slice(6);
      dates = {
    ...
    if (res.reportId){
      let report_url = await this._getReportURL(res.reportId, req_params.operation + '/', req_params.is_v3);
  ...
  async getReport(req_params, is_v3 = false){
    ...
    let res = await this.callAPI(req_params);
    if (res.reportId){
      let report_url = await this._getReportURL(res.reportId, reports[req_params.campaignType].reportPath, is_v3);
      ...
      if (url.searchParams.has('X-Amz-Signature')){
        let filetype = url.pathname.split('/').pop().split('.').pop().toUpperCase();
        let report = await request({
          url:report_url
        });
        this._validateDownloadSuccess(report);
  ...
  async callAPI(req_params, tries = 0){
    ...
    if (![200, 202, 207].includes(res.statusCode)){
      ...
      // Refresh access_token when expired/invalid and auto_request_token is true
      if (this._options.auto_request_token && res.statusCode === 401 && this._isUnauthorized(json_res, req_params.is_v3)){
        await this.refreshAccessToken();
        return await this.callAPI(req_params);
      }
      throw new CustomError({
        code:json_res.code,
        message:json_res.detail
      });
    }
    return json_res;
  }
